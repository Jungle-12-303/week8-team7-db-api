...
- debug sleep 기반 `INSERT` overlap + verification 케이스
...
- 병렬 `SELECT`, 직렬 `INSERT`, 혼합 요청, debug sleep select/insert overlap 검증 시나리오가 준비된다.
...
- debug sleep 기반 `INSERT` overlap 케이스에서는 위 overlap 흔적과 함께 verification 조회가 모두 통과해야 한다.
