...
- `debug_sleep_overlap_insert.json`: `X-Debug-Sleep-Ms`를 사용해 INSERT 요청 수용 단계 overlap과 후행 조회 일관성을 함께 확인한다.
...
- 현재 runtime 기본 lock policy는 `serial_all`이라 실제 DB 실행은 직렬화될 수 있고, API 계층 overlap 시연은 `debug_sleep_overlap_select.json`, `debug_sleep_overlap_insert.json`으로 분리한다.
