...
- `X-Debug-Sleep-Ms`를 이용한 API 계층 overlap 시연 케이스가 `SELECT`, `INSERT` 모두 준비됐다.
- Docker 기반 실제 서버 against 기본 5케이스 재실행까지 완료됐다.
...
- debug sleep 기반 `INSERT` overlap은 API 계층 지연 중첩과 write 결과 일관성을 함께 보여주는 별도 케이스로 분리한다.
